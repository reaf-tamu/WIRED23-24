...
def detect_buoy(results):
	for k in range(5):
		try:
			for j in range(5):
				try:
					if ((results[0].predictions[j].class_name == "buoy") and (results[0].predictions[0].confidence >= 0.75)):
						buoy = 1
...
def detect_gate(results):
	for k in range(5):
		try:
			for j in range(5):
				try:
					if ((results[0].predictions[j].class_name == "gate") and (results[0].predictions[0].confidence >= 0.75)):
						gate = 1
						X = results[0].predictions[j].x
